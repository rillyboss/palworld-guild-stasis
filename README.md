...
| `sweep_interval_ms` | `30000` | Milliseconds between sweeps. Each sweep re-derives all state from live objects, so a longer interval delays reaction without ever desynchronising. Do not set this small: `FindAllOf` scans the entire UObject array every sweep. |
